...
public class App {
	...
	public static void addContact() {
		...
		//Boucle pour saisie d'un numéro de téléphone valide
		while(!Pattern.matches("^(0)[1-9](\\d{2}){4}$",contact.getPhone())){
			System.out.println("Saisisez un numéro");
			contact.setPhone(sc.nextLine());
		}
	...
	public static void editContact() throws SQLException{
		...
		if ( null == contact ) {
			...
			System.out.print( "Entrer l'email ('" + contact.getEmail() + "'; laisser vide pour ne pas mettre à jour) : " );
			String email = sc.nextLine();
			if ( !email.isEmpty() ) {
				contact.setEmail( email );
			}
			System.out.print( "Entrer le téléphone ('" + contact.getPhone() + "'; laisser vide pour ne pas mettre à jour) : " );
	...
	private static void storeContacts() throws NumberFormatException, SQLException{
		...
		do{
			...
			yes = false;
		} while(!Pattern.matches(regex,reponse));
